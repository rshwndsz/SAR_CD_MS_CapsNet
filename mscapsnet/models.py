...
class Conv_Capsule(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.W = self.add_weight(
            shape=[
                1, self.caps_num_i, self.output_cap_channel,
        ...
        self.bias = self.add_weight(
            shape=[1, 1, 1, self.caps_num_i, self.output_cap_channel, 1, 1],
    ...
    def call(self, inputs, training=None):
        ...
        inputs_poses = K.reshape(
            inputs_poses,
            shape=[
                -1, self.kernel_shape[0] * self.kernel_shape[1] *
                self.kernel_shape[2], i_pose_dim
            ])

        votes = self.mat_transform(inputs_poses,
                                   size=self.batchsize * spatial_x * spatial_y)

        votes_shape = votes.shape
        votes = K.reshape(votes,
                          shape=[
                              -1, spatial_x, spatial_y, votes_shape[-3].value,
    ...
    def kernel_tile(self, input):
        ...
        input = tf.reshape(input,
                           shape=[
                               -1, input_shape[1].value, input_shape[2].value,
                               input_shape[3].value * input_shape[4].value
                           ])

        input_shape = input.shape
        tile_filter = np.zeros(shape=[
            self.kernel_shape[0], self.kernel_shape[1], input_shape[3].value,
        ...
        output = tf.nn.depthwise_conv2d(input,
                                        tile_filter_op,
        ...
        output = K.reshape(output,
                           shape=[
                               -1, output_shape[1].value,
                               output_shape[2].value, input_shape[3].value,
                               self.kernel_shape[0] * self.kernel_shape[1]
                           ])
    ...
    def capsules_dynamic_routing(self, votes):
        ...
        for i in range(self.num_routing):
            c = tf.nn.softmax(self.bias,
                              dim=4)  # dim=4 is the num_capsule dimension
        ...
        return K.reshape(outputs, [
            -1, shape[1].value, shape[2].value, shape[4].value, shape[5].value
        ])
...
class Class_Capsule(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.W = self.add_weight(shape=[
            self.input_num_capsule, self.num_capsule, self.input_dim_vector,
            self.dim_vector
        ],
    ...
    def call(self, inputs, training=None):
        ...
        for i in range(self.num_routing):
            ...
            if i == self.num_routing - 1:
                ...
                outputs = squash(
                    K.sum(c * inputs_hat_stopped, 1, keepdims=True))
...
def PrimaryCap1(inputs, dim_vector, n_channels, kernel_size, strides, padding):
    ...
    outputs = layers.Reshape(
        target_shape=[shape[1].value, shape[2].value, n_channels, dim_vector],
...
def PrimaryCap2(inputs, dim_vector, n_channels, kernel_size, strides, padding):
    ...
    output = layers.BatchNormalization(momentum=0.9,
                                       name='primarycap_bn_1')(output)
    output = layers.Activation('relu', name='primarycap_relu_1')(output)
    shape = np.shape(output)

    outputs = layers.Reshape(
        target_shape=[shape[1].value, shape[2].value, n_channels, dim_vector],
        name='primarycap_reshape_1')(output)
...
def AFC_layer(x):
    ...
    conv3 = layers.Conv2D(filters=32,
                          kernel_size=3,
                          dilation_rate=3,
                          strides=1,
                          padding='same',
    ...
    conv1_d = layers.Conv2D(filters=32,
                            kernel_size=1,
                            strides=1,
                            padding='same',
                            name='conv1_1')(conv1)
    conv2_d = layers.Conv2D(filters=32,
                            kernel_size=1,
                            strides=1,
                            padding='same',
                            name='conv2_1')(conv2)
